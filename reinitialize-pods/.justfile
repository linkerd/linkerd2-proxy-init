# This justfile includes recipes for building and packaging
# linkerd-reinitialize-pods for release. This file is separated so that we can
# invoke cargo, etc when building defaults. If this logic were in the top-level
...
version := `just-cargo crate-version linkerd-reinitialize-pods`
...
_target-bin := _target-dir / "linkerd-reinitialize-pods"

_package-name := "linkerd-reinitialize-pods-" + version + "-" + arch
_package-tgz := "../target/package" / _package-name + ".tgz"
_package-dir := "../target/package" / _package-name
_package-bin := _package-dir / "linkerd-reinitialize-pods"
...
build *flags:
    {{ _cargo }} fetch --locked
    {{ _cargo }} build --workspace -p linkerd-reinitialize-pods {{ flags }}
