...
// will result in
//   []int{23,25,26,27}
func MapRangeSlice(target *[]int, source []string) error {
	*target = make([]int, 0)
	for _, portOrRange := range source {
		if strings.Contains(portOrRange, "-") {
			...
			lower, err := strconv.ParseInt(bounds[0], 10, 0)
			if err != nil {
				return fmt.Errorf("\"%s\" is not a valid lower-bound", bounds[0])
			}
			upper, err := strconv.ParseInt(bounds[1], 10, 0)
			if err != nil {
			...
			i, err := strconv.ParseInt(portOrRange, 10, 0)
			if err != nil {
				return err
			}
			*target = append(*target, int(i))
