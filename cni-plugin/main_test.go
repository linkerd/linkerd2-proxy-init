...
func TestCmdAddLogsParseConfigErrorsClearly(t *testing.T) {
	...
	output := logs.String()
	if !strings.Contains(output, "error parsing config:") {
		t.Fatalf("expected parse error log, got %q", output)
	}
	if !strings.Contains(output, "failed to parse network configuration") {
		t.Fatalf("expected underlying parse error in log, got %q", output)
	}
